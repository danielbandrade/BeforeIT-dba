...
Bit.@object mutable struct Properties(Object) <: AbstractProperties
    ...
    psi::Bit.typeFloat
    psi_H::Bit.typeFloat
    # Adding gambling market parameters
    gambling_income_share::Bit.typeFloat
    gambling_active_worker_ids::Vector{Bit.typeInt}
    gambling_inactive_worker_ids::Vector{Bit.typeInt}
    gambling_owner_ids::Vector{Bit.typeInt}
    mu::Bit.typeFloat
...
function Properties(parameters::Dict{String, Any}, initial_conditions)
    ...
    gambling_active_worker_ids = Vector{typeInt}(get(initial_conditions, "gambling_active_worker_ids", Int[]))
    gambling_inactive_worker_ids = Vector{typeInt}(get(initial_conditions, "gambling_inactive_worker_ids", Int[]))
    gambling_owner_ids = Vector{typeInt}(get(initial_conditions, "gambling_owner_ids", Int[]))
    if gambling_income_share < 0 || gambling_income_share > 1
        throw(ArgumentError("gambling_income_share must be between 0 and 1"))
    end

    H_W = H_act - I - 1

    if !allunique(gambling_active_worker_ids)
        throw(ArgumentError("gambling_active_worker_ids must contain unique IDs"))
    end

    if !all(id -> 1 <= id <= H_W, gambling_active_worker_ids)
        throw(ArgumentError("gambling_active_worker_ids contains an invalid ID"))
    end

    if !allunique(gambling_inactive_worker_ids)
        throw(ArgumentError("gambling_inactive_worker_ids must contain unique IDs"))
    end

    if !all(id -> 1 <= id <= H_inact, gambling_inactive_worker_ids)
        throw(ArgumentError("gambling_inactive_worker_ids contains an invalid ID"))
    end

    if !allunique(gambling_owner_ids)
        throw(ArgumentError("gambling_owner_ids must contain unique IDs"))
    end

    if !all(id -> 1 <= id <= I, gambling_owner_ids)
        throw(ArgumentError("gambling_owner_ids contains an invalid ID"))
    end

    if gambling_income_share > 0
        if isempty(gambling_active_worker_ids) &&
                isempty(gambling_inactive_worker_ids)
            throw(ArgumentError("positive gambling_income_share requires at least one gambler"))
        end

        if isempty(gambling_owner_ids)
            throw(ArgumentError("positive gambling_income_share requires at least one owner"))
        end
    end
    ...
    return Properties(
        ...
        tau_SIW, tau_EXPORT, tau_CF, tau_G, theta_UB, psi, psi_H, gambling_income_share,
        gambling_active_worker_ids, gambling_inactive_worker_ids, gambling_owner_ids, mu,
        theta_DIV, theta, zeta, zeta_LTV, zeta_b, b_CF_g, b_CFH_g, b_HH_g, c_G_g, c_E_g,
        c_I_g, a_sg, C, D_H, K_H, sb_other, E_k, r_bar,
    )
end
