...
function households_budget_act(model::AbstractModel)
    ...
    income_act = households_income_act(model; expected = true)

    stakes_act, _, _ = gambling_transfers(model; income_act)

    Y_e_h = income_act - stakes_act
...
function gambling_transfers(
        model::AbstractModel;
        income_act = nothing,
        income_inact = nothing,
    )
    stakes_act = zeros(typeFloat, length(model.w_act))
    stakes_inact = zeros(typeFloat, length(model.w_inact))
    receipts_firms = zeros(typeFloat, length(model.firms))

    share = model.prop.gambling_income_share

    if iszero(share)
        return stakes_act, stakes_inact, receipts_firms
    end

    if !isnothing(income_act)
        active_ids = Set(model.prop.gambling_active_worker_ids)

        for (index, id) in pairs(model.w_act.ID)
            if id in active_ids
                stakes_act[index] =
                    share * max(zero(typeFloat), income_act[index])
            end
        end
    end

    if !isnothing(income_inact)
        inactive_ids = Set(model.prop.gambling_inactive_worker_ids)

        for (index, id) in pairs(model.w_inact.ID)
            if id in inactive_ids
                stakes_inact[index] =
                    share * max(zero(typeFloat), income_inact[index])
            end
        end
    end

    total_stakes = sum(stakes_act) + sum(stakes_inact)
    owner_ids = Set(model.prop.gambling_owner_ids)
    receipt = total_stakes / length(owner_ids)

    for (index, id) in pairs(model.firms.ID)
        if id in owner_ids
            receipts_firms[index] = receipt
        end
    end
...
function set_gambling_transfers!(model::AbstractModel)
    stakes_act, stakes_inact, receipts_firms = gambling_transfers(
        model;
        income_act = model.w_act.Y_h,
        income_inact = model.w_inact.Y_h,
    )

    model.w_act.Y_h .-= stakes_act
    model.w_inact.Y_h .-= stakes_inact
    model.firms.Y_h .+= receipts_firms
    model.agg.gambling_volume = sum(stakes_act) + sum(stakes_inact)
...
function households_budget_inact(model::AbstractModel)
    ...
    income_inact = households_income_inact(model; expected = true)
    _, stakes_inact, _ = gambling_transfers(model; income_inact)

    Y_e_h = income_inact - stakes_inact
...
function households_budget_firms(model::AbstractModel)
    psi, psi_H, tau_VAT, tau_CF =
        model.prop.psi, model.prop.psi_H, model.prop.tau_VAT, model.prop.tau_CF
    ...
    Y_e_h =
        households_income_firms(model; expected = true) +
        receipts_firms
