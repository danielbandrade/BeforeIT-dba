...
Bit.@object struct Data(Object) <: AbstractData
    ...
    gambling_volume::Vector{Bit.typeFloat} = Bit.typeFloat[]
end
...
function allocate_new_data!(m::AbstractModel)
    ...
    return push!(d.gambling_volume, 0.0)
end
...
function update_data_init!(m::AbstractModel)
    ...
    d.gambling_volume[1] = zero(typeFloat)
    return m
end
...
function update_data_step!(m::AbstractModel)
    ...
    d.gambling_volume[t] = m.agg.gambling_volume
    return m
end
